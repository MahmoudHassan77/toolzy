    ...
    items: [
      ...
      },
      {
        ...
        icon: (
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
              d="M12 4h.01M4 4h4v4H4V4zm12 0h4v4h-4V4zM4 16h4v4H4v-4zm8 4h.01M16 16h4v4h-4v-4zM8 8h.01M16 8h.01M8 16h.01" />
          </svg>
        ),
      },
      {
...
export default function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <aside
      className={`
        fixed inset-y-0 left-0 z-30 w-64 bg-surface text-fg2 flex flex-col border-r border-line
        transition-transform duration-200 ease-in-out
        ${open ? 'translate-x-0' : '-translate-x-full'}
        md:relative md:w-60 md:translate-x-0 md:shrink-0
      `}
    >
      <div className="px-5 py-5 border-b border-line shrink-0 flex items-center justify-between">
        <span className="text-acc font-bold text-lg tracking-tight">Toolzy</span>
        {/* Close button — mobile only */}
        <button
          onClick={onClose}
          className="md:hidden w-8 h-8 flex items-center justify-center rounded-lg text-fg2 hover:bg-raised hover:text-fg1 transition-colors"
      ...
      <nav className="flex-1 py-3 overflow-y-auto">
        {navSections.map((section, si) => (
          <div key={si} className={si > 0 ? 'mt-3 pt-3 border-t border-line' : ''}>
            {section.heading && (
              <p className="text-[10px] font-semibold uppercase tracking-widest text-fg3 px-6 mb-1">
                {section.heading}
              </p>
            )}
